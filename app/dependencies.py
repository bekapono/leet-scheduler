...
class ProblemServiceDependency:
    def __init__(self, session: ):
        self.repository = ProblemRepository(SessionLocal())
        self.problem_service = ProblemService(self.repository)
'''

def problem_service_factory(session: Session = Depends(get_db)) -> ProblemService:
...
def scheduler_service_factory(session: Session = Depends(get_db)) -> SchedulerService:
